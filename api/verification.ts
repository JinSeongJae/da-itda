...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // TEMP(2026-08-31): 일회성 진단용, 읽기 전용 — 사용 직후 제거할 것.
  if (req.method === 'GET' && req.query.resource === 'debug-status') {
    const users = await query<{ id: string; name: string; profile: unknown }>(
      'SELECT id, name, profile FROM app_users ORDER BY created_at DESC'
    );
    const threads = await query<{ id: string; user_a_id: string; user_b_id: string; created_at: string }>(
      'SELECT id, user_a_id, user_b_id, created_at FROM threads ORDER BY created_at DESC'
    );
    res.status(200).json({
      users: users.map((u) => ({ id: u.id, name: u.name, onboarded: u.profile != null })),
      threads,
    });
    return;
  }
